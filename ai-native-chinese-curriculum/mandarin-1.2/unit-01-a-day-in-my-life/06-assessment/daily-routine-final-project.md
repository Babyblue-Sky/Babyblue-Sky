---
assessment_type: "Summative — Performance Task / Project"
title: "健康还是不健康？(Healthy or Unhealthy?) — Daily Routine Final Project"
unit: Unit 1
status: canonical
standards:
  - Presentational Communication
grading_scale: "EE (Exceeding) / ME (Meeting) / AE (Approaching) / BE (Below)"
source: "2026-08-04 uploaded: Final_Project_Daily_Routine.docx (instructions/vocab/rubric),
  Final_Draft_graded.docx (blank rubric/grading grid, teacher confirmed OK to reference directly
  since this is Project-type — see 08-curriculum-intelligence.md on quiz vs. project content
  exposure). 2026-08-05: dropped the three HAL.N (Habits of Learning) standards and the
  MS.WL.3 code — HAL is a school-specific rubric axis unrelated to language proficiency, and
  the WL numbering won't apply once the teacher is elsewhere; also dropped the specific due
  dates from Instructions (no calendar dates on the page)."
last_updated: 2026-08-05
---

# 健康还是不健康？(Healthy or Unhealthy?) — Daily Routine Final Project

## Driving Question
健康还是不健康？(Healthy or unhealthy?) — What does a healthy vs. an unhealthy daily routine
look like, and which one do you want to live?

## Instructions
We've learned how to describe our routines and what healthy vs. unhealthy lifestyles look like.
For the final project, create an E-book in Chinese characters covering:
- A short story about a character's healthy lifestyle (健康的生活方式)
- A short story about a character's unhealthy lifestyle (不健康的生活方式)
- What lifestyle you'd want to live, and why (我喜欢…因为…，at least three reasons)

Process:
1. Plan the project (bullet-point your ideas for each of the three parts above).
2. Draft by hand in paragraphs — Part 1 (healthy routine), Part 2 (unhealthy routine), Part 3 (which you'd choose and why).
3. Type your final draft in a Word document.
4. Build the E-book at bookcreator.com — sign in with your school Google account, join the class's Online Mandarin Library, then design your book (comic-style layout recommended for more design options).
5. Make sure the finished book has: a title, your name as author, all sentences from your final draft, and a picture on every page (or every other page).
6. Record yourself reading every page aloud.
7. Publish.

## Vocabulary
| 中文 | 拼音 | 意思 |
|---|---|---|
| 生活方式 | shēng huó fāng shì | lifestyle |
| 健康 | jiàn kāng | healthy |
| 沙拉 | shā lā | salad |
| 可乐 | kě lè | cola |
| 水 | shuǐ | water |
| 薯片 | shǔ piàn | chips |
| 水果 | shuǐ guǒ | fruit |
| 蔬菜 | shū cài | vegetables |
| 看电视 | kàn diàn shì | to watch TV |
| 做 | zuò | to do |
| 工作 | gōng zuò | to work |
| 东西 | dōng xi | thing(s) |
| …的时候 | ...de shí hou | when... / at the time of... |
| 很多 | hěn duō | a lot / many |
| 胖 | pàng | fat |
| 瘦 | shòu | thin |
| 然后 | rán hòu | then |
| 汉堡 | hàn bǎo | hamburger |
| 披萨 | pī sà | pizza |
| 面条 | miàn tiáo | noodles |
| 健身 | jiàn shēn | to work out |
| 水饺 | shuǐ jiǎo | dumplings |
| 很早 | hěn zǎo | very early |
| 很晚 | hěn wǎn | very late |

## Rubric
Scored EE (Exceeding) / ME (Meeting) / AE (Approaching) / BE (Below) on:
- **Presentational Communication** — presents information/ideas on a variety of topics, adapted to the audience, to describe/inform/narrate/explain/persuade

## Resources
- E-book builder: bookcreator.com (sign in with school Google account, join class library via class code)
- Sample work from previous years is shown in class (not included in this Content Layer file — physical/in-class samples, not a shareable link)

## Examples
Ms. Liao's own model text ("聊老师的一天" / "Ms. Liao's Day"), used in class as a reading model
before students draft their own (see [Cycle 7](../02-teaching/cycle-7.md)):

> 我每天早上六点起床，刷牙，然后我在家吃早饭。我早饭喜欢吃鸡蛋🥚和牛奶🥛。我早上七点去学校。
> 上午八点半，我上课。中午十一点五十分，我在学校吃饭。我喜欢吃学校的午饭，因为 Chef 做的饭很
> 健康。然后下午我上课。我下午四点下班，然后我回家。我喜欢下午五点跑步，然后我洗澡，吃晚饭。
> 我喜欢吃健康的晚饭。晚上七点我工作，然后晚上十点我去睡觉。

("Every day I get up at 6am, brush my teeth, then eat breakfast at home. I like eggs and milk for
breakfast. I leave for school at 7am, class starts at 8:30, and I eat lunch at school at 11:50 —
I like the school lunch because the chef's food is healthy. I have afternoon classes, get off
work at 4pm, then go home. I like to run at 5pm, then shower and eat dinner — I like to eat a
healthy dinner. I work at 7pm, then go to sleep at 10pm.")

No completed student example is available yet — `Final_Draft_graded.docx` is a blank rubric/grading
grid template, not a filled sample.

## Reflection
> 待补充
